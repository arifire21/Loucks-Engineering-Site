//dining
import Lucky_Fish from '@/images/lucky_fish.jpg'
import Oceanic from '@/images/oceanic.jpg'
import Baresco from '@/images/baresco.jpg'
import CarouselClub from '@/images/carousel-club.jpg'
import CubicoCoffee from '@/images/cubico-coffee-5275.jpg'
import Cove from '@/images/thecove.jpg'
//import MoraGrill from '@/images/mora-grill.jpg'
import HYB from '@/images/HYB.png'
import Pinball from '@/images/pinball.jpg'

//residence
import BluApts from '@/images/blu-apt.jpg'

//offices
// import Broward from '@/images/200EBroward_crop.jpg'
import Broward2 from '@/images/broward-aerial.avif'
import Esperante from '@/images/Esperante.jpg'
import Broward500 from '@/images/500EBroward.webp'
import OneBoca from '@/images/onebocaplace.jpg'
import Lakeshore from '@/images/lakeshore.jpg'

//outreach
import SheridanHouse from '@/images/sheridan-house.jpg'
import Goodwill from '@/images/goodwill.png'
import YMCA from '@/images/ymca-boca.jpg'
import BGC from '@/images/boys-and-girls-club.jpg'

//aviation
import AvengerWarehouse from '@/images/afg-warehouse.jpeg'
import AvengerSpirit from '@/images/afg-spirit.jpeg'
import Delta from '@/images/Miami-Delta-Skyclub-1.jpg'
import Sheltair from '@/images/Sheltair-FLLjpg.jpg'

//auto
import PBGarage from '@/images/pbgarage.jpg'
import NissanMez from '@/images/Nissan_Mez.jpg'

//education
import CGLocker from '@/images/cardinalgibbons.webp'
import FIUStudent from '@/images/FIU_Student_Academic_Center_c87uka.webp'
import Azure from '@/images/azurecollege.png'

//pets
import FurRay from '@/images/furray.jpg'
import PetLife from '@/images/petlife.jpg'

//gyms
import UFCGym from '@/images/ufc-gym.jpg'
import Athletica from '@/images/athletica.png'
import F45Training from '@/images/f45-training.jpg'
import AirLab from '@/images/airlab.png'

//worship
import FBChurch from '@/images/PompanoBchFBC.jpg'
import Gregory from '@/images/gregory.jpg'
import FPChurch from '@/images/first-presbyterian.png'
import MaryMag  from '@/images/Saint-Mary-Magdalene-St.-Martin-Episcopal-Church2.jpg'
import Coleman from '@/images/colemans.jpg'
import Calvary from '@/images/calvary.jpg'

//beauty
import Venetian from '@/images/venetian.png'
import Barber from '@/images/carved-barber.jpg'
import HairBenders from '@/images/hair-benders.jpg'
import MulanSpa from '@/images/mulan-spa.jpg'

//medDental
import Cohen from '@/images/cohen-chiro.jpg'
import PathMedical from '@/images/path-medical.jpg'
import Groves from '@/images/groves-medical-plaza-webp.webp'
import Pediatrix from '@/images/pediatrix.jpg'

//storage
import LuckyPick from '@/images/luckypick.jpg'
import ExtraSpace from '@/images/extra space storage.webp'
import Storaway from '@/images/storaway.jpg'
import Fayard from '@/images/john-fayard-storage.jpg'

//retail
import Shoppes from '@/images/broward-shoppes.jpg'
import Stirling from '@/images/The-Shoppes-at-Stirling-Place.jpg'
import MacysVestibule from '@/images/macys-boca.jpg'

//laundry
import Aaxon from '@/images/aaxon-laundry.jpg'
import PalmLaundry from '@/images/palm-aire-laundry.jpg'
import JetLaundry from '@/images/Jet-Laundry-Coral-Springs.jpg'

//supermarkets
import Presidente from '@/images/presidente.jpg'
import WholeFoods from '@/images/whole-foods.jpg'
import Bodegon from '@/images/bodegon.jpg'

//multipurpose
import Mizner from '@/images/mizner-park-fountain.jpg'

//other
import Jumbotron from '@/images/gulfstream.jpg'

//cleanrooms
import AirportLogistics from '@/images/Airport-Logistics.jpg'

export const restaurants = [
    {
        name: "Lucky Fish (Pompano Bch)",
        website: "https://www.luckyfishpompano.com/",
        image: Lucky_Fish,
        year: 2018,
        arch: 'RWB Architects',
        coords: [26.2354, -80.08897229805034]
    },
    {
        name: "Oceanic",
        website: "https://www.oceanicpompano.com/",
        image: Oceanic,
        year: 2016,
        arch: 'RWB Architects',
        coords: [26.23581157785526, -80.08904317461128]
    },
    {
        name: "Baresco",
        website: "https://eatbaresco.com/",
        image: Baresco,
        year: 2021,
        arch: 'CSA',
        coords: [26.235869639172, -80.08943552222465]
    },
    {
        name: "Beach House",
        smallText: "(kitchen)",
        website: "https://beachhousepompano.com/"
    },
    {
        name: "How You Brewin'",
        website: "https://howyoubrewin.com/pages/pompano-guide-for-guests",
        image: HYB,
        year: 2022,
        arch: 'CSA',
        coords: [26.236697283043124, -80.09017001428346]
    },
    {
        name: "Beach House",
        smallText: "(kitchen)",
        website: "https://beachhousepompano.com/"
    },
    {
        name: "Satellite Pinball Lounge",
        website: "https://www.satellitepinballlounge.com/",
        year: 2024,
        image: Pinball
    },
    {
        name: "Lucky Fish (Dania Bch)",
        website: "https://www.luckyfishbeachbar.com/dania-beach",
        year: 2025
        //image: Lucky_Fish
    },
    {
        name: "Carousel Club",
        website: "https://carouselclub.com",
        image: CarouselClub,
        coords: [25.980103968915184, -80.13892428265447]
    },
    {
        name: 'Cubico Coffee',
        website: 'https://www.cubicocoffee.com/',
        image: CubicoCoffee,
        year: 2022,
        arch: "BG Architects",
        coords: [26.06055934771893, -80.34788717461608]
    },
    {
        name: 'The Cove',
        website: 'https://www.thecovedeerfield.com/',
        image: Cove,
        year: 2021,
        arch: 'RWB Architects',
        coords: [26.313747619370787, -80.08158147460914]
    }
]

export const residences = [
    {
        name: "Blu on Marina Boulevard",
        website: 'https://www.bluonmarinaboulevard.com',
        image: BluApts,
        coords: [26.091875405138612, -80.1591043355374]
    },
    {
        name: 'Pompano Pointe Apts',
        year: '2022-2024',
        arch: 'S.H. Brandt',
        coords: [26.26998410261203, -80.15031022532283]
    }
]

export const offices = [
    {
        name: "200 E. Broward",
        website: "https://www.200eastbroward.com/",
        year: '2019-2024',
        arch: 'Various',
        image: Broward2,
        coords: [26.12222946027434, -80.14089684396279]
    },
    {
        name: '500 E. Broward/Broward Financial Center',
        website: 'https://www.browardfinancialcentre.com/',
        year: '2019-2024',
        arch: 'Various',
        image: Broward500,
        coords: [26.12210830745826, -80.13821333807186]
    },
    {
        name: 'One Boca Place',
        website: 'https://www.onebocaplace.com/',
        image: OneBoca,
        year: '2005-2020',
        arch: 'Various',
        coords: [26.369137982405746, -80.1263570658463]
    },
    {
        name: 'YMP Real Estate Office',
        website: 'https://ymprealestate.com/',
        year: 2022,
        arch: 'TD Architect Group',
        image: Lakeshore,
        coords: [26.181950555257696, -80.20246546033366]
    },
    {
        name: "Esperante",
        year: '2014-2024',
        arch: 'Various',
        image: Esperante,
        coords: [26.705942659378785, -80.0508708105364]
    }
]

export const outreach = [
    {
        name: "Sheridan House",
        website: "https://sheridanhouse.org/",
        image: SheridanHouse,
        year: '2004-2007, 2023-2024',
        arch: 'Pierce, Claren',
        coords: [26.099171793722164, -80.31198308364891]
    },
    {
        name: "Goodwill",
        smallText: '(kitchen)',
        website: "https://goggi.org/",
        image: Goodwill,
        year: 2022,
        arch: 'Myott Architecture',
        coords: [26.754522554218198, -80.08153330175482]
    },
    {
        name: 'YMCA',
        smallText: '(Boca Raton)',
        website: 'https://www.ymcaspbc.org/locations/peter-blum-family-ymca-boca-raton',
        image: YMCA,
        year: '2022-2023',
        arch: 'CSA',
        coords: [26.352992766143526, -80.14832193752302]
    },
    {
        name: 'Boys and Girls Club',
        image: BGC,
        year: '2011-2023',
        arch: 'AAAdvance',
        coords: [0,0]
    }
]

export const aviation = [
    {
        name: 'Avenger Flight Group - Warehouse',
        website: 'https://www.afgsim.com/flight-training-centers/north-america/fort-lauderdale/',
        image: AvengerWarehouse,
        year: 2021,
        arch: 'RLC',
        coords: [26.07115210763237, -80.15936648999622]
    },
    {
        name: 'Avenger Flight Group - Spirit Pilot Training Room',
        website: 'https://www.afgsim.com/flight-training-centers/north-america/fort-lauderdale/',
        image: AvengerSpirit,
        year: 2022,
        arch: 'RLC',
        coords: [26.07115210763237, -80.15936648999622]
    },
    {
        name: 'Sheltair Aviation Office',
        website: 'https://www.sheltairaviation.com/locations/fll/',
        image: Sheltair,
        year: 2022,
        arch: 'Cartaya',
        coords: [26.069934924209957, -80.1559133672749]
    },
    {
        name: "Vero Beach Airport Hangars",
        year: '2023-2024',
        arch: 'BG Architects',
        coords: [27.653376952733957, -80.40611121694016]
    },
    {
        name: "Delta In-Flight Lounge - MIA Airport",
        website: "https://www.delta.com/us/en/airports/united-states/miami-sky-club-airport-map#dscPartner",
        image: Delta,
        year: 2019,
        arch: 'Cartaya',
        coords: [25.792871238658037, -80.28231662893995]
    }
]

export const education = [
    {
        name: 'FIU Student Center',
        smallText: '(offcampus building)',
        website: 'https://www.fiu.edu/',
        image: FIUStudent,
        year: 2022,
        coords: [25.756287710953714, -80.37548076656765]
    },
    {
        name: 'Azure College',
        website: 'https://azure.edu/',
        image: Azure,
        year: 2020,
        coords: [26.187841908133514, -80.18990062890606]
    },
    {
        name: 'Cardinal Gibbons HS',
        smallText: '(locker room & misc.)',
        website: 'https://www.cghsfl.org/',
        image: CGLocker,
        coords: [26.185956053116602, -80.10876581551804]
    },
    {
        name: 'Palm Beach School for Autism',
        website: 'https://pbsfa.org/',
        coords: [26.590519977962746, -80.18065589066028]
    }
]

export const gyms = [
    {
        name: "AIRLAB Fitness",
        website: 'https://www.airlabfitness.com/',
        image: AirLab,
        year: 2022,
        coords: [25.805033392791557, -80.19278038305247]
    },
    {
        name: 'UFC Gym',
        image: UFCGym,
        year: 2014,
        coords: [25.811716480460277, -80.19295204662541]
    },
    {
        name: 'F45 Training',
        website: 'https://f45training.com/thecove/home',
        image: F45Training,
        year: 2020,
        coords: [26.307942489446983, -80.09215152885272]
    },
    {
        name: 'Athletic Health and Fitness',
        website: 'https://www.athleticahf.com/sunrise',
        image: Athletica,
        year: 2022,
        coords: [26.170917599485108, -80.2974562828916]
    }
]

export const worship = [
    {
        name: 'First Baptist Church',
        smallText: ' (restrooms)',
        website: 'https://www.fbcpompano.org/,',
        image: FBChurch,
        year: 2021,
        coords: [26.232178083956143, -80.12318708465932]
    },
    {
        name: `St. Gregory's Episcopal Church`,
        website: 'https://www.stgregorysepiscopal.org/',
        image: Gregory,
        year: 2022,
        coords: [26.351868593093677, -80.08279685824135]
    },
    {
        name: 'First Presbyterian Church',
        image: FPChurch,
        year: 2021,
        coords: [26.261137895002097, -80.09835752887358]
    },
    {
        name: 'Episcopal Church of St. Mary Magdalene & St. Martin',
        website: 'https://stmmcs.net',
        image: MaryMag,
        year: 2023,
        coords: [26.248018147395168, -80.24154627996083]
    },
    {
        name: 'St. Coleman Catholic Church',
        website: 'https://www.saintcoleman.org/',
        image: Coleman,
        coords: [26.215667353818173, -80.10517530616822]
    },
    {
        name: 'Calvary Chapel (Parkland)',
        website: 'https://calvaryftl.org/campus/parkland/',
        image: Calvary,
        year: 2022,
        coords: [26.287896498449577, -80.20056390556518]
    }
]

export const medDental = [
    {
        name: 'Cohen Chiropractic and Wellness',
        website: 'https://www.cohenchiropracticandwellness.com/',
        image: Cohen,
        year: 2021,
        coords: [26.168459933999287, -80.11294447550776]
    },
    {
        name: 'Path Medical',
        website: 'https://pathmedical.com/locations/pembroke-pines/',
        image: PathMedical,
        year: 2021,
        coords: [26.014760553176778, -80.34508653761148]
    },
    {
        name: 'Groves Medical Plaza',
        image: Groves,
        year: 2021,
        coords: [0,0]
    },
    // {
    //     name: 'Pediatrix',
    //     image: Pediatrix,
    //     coords: [0,0]
    // }
]

export const beauty = [
    {
        name: 'Venetian Nail Spa',
        smallText: '(multiple locations)',
        website: 'https://venetiansalon.com/',
        image: Venetian,
        year: 'Various years',
        coords: [26.263423995177387, -80.10043743848196]
    },
    {
        name: 'Carved Barber Shop',
        website: 'https://carvedbarbershop.com/',
        image: Barber,
        year: 2022,
        coords: [26.160085956279318, -80.13083837643518]
    },
    {
        name: 'Hair Benders Salon',
        website: 'https://www.hairbenderssalon.com/',
        image: HairBenders,
        year: 2022,
        coords: [36.537040701460256, -82.51725255907911]
    },
    {
        name: "Mulan Spa",
        website: 'https://www.mulanspa.com/',   
        image: MulanSpa
    }
]

export const pets = [
    {
        name: 'Hip Hip FurRay!',
        website: 'https://www.hiphipfurray.com/',
        image: FurRay,
        year: 2021,
        coords: [26.51686404933353, -80.0593476134189]
    },
    {
        name: 'PetLife Animal Hospital',
        website: 'https://petlifefl.com/',
        image: PetLife,
        year: 2021,
        coords: [26.710312027559056, -80.15010693309858]
    }
]

export const laundry = [
    {
        name: 'Aaxon Laundry Systems',
        website: 'https://www.aaxon.com/multi-housing',
        image: Aaxon,
        coords: [26.29254571015873, -80.15374884023322]
    },
    {
        name: 'Sock Monster Laundromart',
        smallText: '(tenent improvement)',
        website: 'https://www.sockmonsterlaundromart.com/',
        image: PalmLaundry,
        year: 2020,
        coords: [26.230048056176113, -80.15797477548051]
    },
    {
        name: 'Jet Laundry',
        website: 'http://www.jetcoinlaundromat.com/',
        image: JetLaundry,
        year: 2020,
        coords: [26.242839343888672, -80.26832315217834]
    }
]

export const storage = [
    {
        name: 'Lucky Pick Self Storage',
        website: 'https://www.luckypickselfstorage.com/',
        image: LuckyPick,
        year: 2020,
        coords: [42.234906601144814, -71.84934018920434]
    },
    {
        name: 'Extra Space Storage',
        website: 'https://www.extraspace.com/storage/facilities/us/florida/tamarac/7764/',
        image: ExtraSpace,
        year: 2021,
        coords: [26.194679725722125, -80.21843685454893]
    },
    {
        name: 'Storaway Self Storage',
        website: 'https://www.storawayselfstorage.com/self-storage/fl/palm-bay/palm-bay-self-storage/',
        image: Storaway,
        year: 2022,
        coords: [28.007640794683056, -80.67365338205227]
    },
    {
        name: 'Fayard Self Storage',
        website: 'https://www.johnfayardselfstorage.com/',
        image: Fayard,
        year: 2022,
        coords: [30.375001414075317, -86.26145714225667]
    }
]

export const auto = [
    {
        name: 'Pompano Beach Parking Garage',
        smallText: '(tenant improvement)',
        website: 'https://www.pompanobeachfl.gov/residents/parking/parking-garage',
        image: PBGarage,
        year: 2020,
        coords: [26.23622474775927, -80.08963270261177]
    },
    {
        name: 'Weston Nissan Volvo',
        smallText: '(mezzanine)',
        website: 'https://www.westonnissan.com/',
        image: NissanMez,
        year: 2020,
        coords: [26.0736092412093, -80.36109595877953]
    }
]

export const retail = [
    {
        name: 'Shoppes of Broward',
        smallText: '(facade renovation)',
        website: 'https://janourarealty.com/property/shoppes-at-broward/',
        image: Shoppes,
        year: 2023,
        coords: [26.120274455340617, -80.25747682950694]
    },
    {
        name: 'Shops at Stirling Place',
        smallText: '(facade renovation)',
        website: 'https://janourarealty.com/property/shops-at-stirling-place/',
        image: Stirling,
        year: 2023,
        coords: [26.046620215075595, -80.2374979846593]
    },
    {
        name: `Macy's`,
        smallText: '(vestibule addition)',
        website: 'https://l.macys.com/boca-raton-fl',
        image: MacysVestibule,
        year: 2019,
        coords: [26.366636296311817, -80.1315993907604]
    }
]

export const supermarkets = [
    {
        name: 'Presidente Supermarket',
        website: 'http://presidentesupermarkets.com',
        image: Presidente,
        year: 2023,
        coords: [26.228804267952444, -80.15822353684385]
    },
    {
        name: 'Whole Foods',
        website: 'https://www.wholefoodsmarket.com/stores/coralsprings',
        image: WholeFoods,
        coords: [26.243214296126823, -80.256658306156]
    },
    {
        name: 'El Bodegón Supermarket #7',
        website: 'https://bodegonfl.com/bodegon-7/',
        image: Bodegon,
        year: '2023-2024',
        coords: [26.69379805665021, -80.2047330907886]
    }
]

export const multipurpose = [
    {
        name: 'Mizner Park',
        smallText: '(tenant improvement, multiple)',
        website: 'https://www.miznerpark.com/en.html',
        image: Mizner,
        year: '2000-2023',
        coords: [26.35418735216275, -80.08487031878359]
    },
    {
        name: 'Fun Sweets Warehouse',
        smallText: '(tenant improvement)',
        website: 'http://funsweets.net/',
        year: 2020,
        coords: [26.685041538695813, -80.21275428465931]
    }
]

export const other = [
    {
        name: "Gulfstream Racing Jumbotron",
        website: "https://gulfstreampark.com/",
        image: Jumbotron,
        year: 2020,
        coords: [25.97821969403824, -80.13958343228856]
    },
    {
        name: 'Pick-A-Ball',
        smallText: '(interactive flooring)',
        website: 'http://pickaballcourts.com/',
        coords: [26.30306061909846, -80.15367131693922],
        year: '2022-2024',
        arch: 'RWB'
    },
    {
        name: "FL Panthers' Training Facility",
        website: 'https://panthersiceden.com/',
        year: 2007,
        coords: [26.271310824614506, -80.2920759611157]
    },
    {
        name: 'Sundance Marina',
        smallText: '(dock electrical service)',
        coords: [28.179039349196223, -80.6503942745556],
        year: 2023,
        arch: 'Sunbelt Electric'
    }
]

export const cleanrooms = [
    {
        name: "GummyWorks",
        website: "https://www.gummyworks.com/",
        year: 2023,
        image: AirportLogistics,
        imagesrc: "Verdex Construction",
        imgsrcLink: "https://verdex.com/airport-logistics-warehouse/"
    },
    {
        name: "TranspireBio (Weston, FL)",
        website: "https://www.transpirebio.com/",
        year: 2024,
    },
    {
        name: "Sincerius Pharmaceuticals Florida",
        website: "https://decathloncapital.com/sincerus-pharmaceuticals/",
        year: 2024,
    },
    {
        name: "Patient Care America",
        website: "https://pcacorp.com/",
        year: 2024,
    },
    {
        name: "MSC TransDermal",
        year: 2019,
    },
    {
        name: "DPR Pharmacy",
        website: "https://dprpharmacy.com/",
        year: 2022,
    }
]